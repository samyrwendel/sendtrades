...
export const mexcExchange = {
    ...
    async executeTrade(credentials, tradeParams) {
        try {
            ...
            const orderCalc = await calculateOrderQuantity(balances, tradeParams.amount, tradeParams.symbol, currentPrice);
            if (orderCalc.error) {
                throw new Error(orderCalc.error);
            }
            // Usar o timestamp fornecido ou obter do servidor
            const timestamp = tradeParams.timestamp || await getServerTime();
            ...
            const orderParams = {
                ...
                recvWindow: '10000' // 10 segundos para compensar possíveis delays
            };
            ...
            if (tradeParams.type.toUpperCase() === 'BUY') {
                orderParams.quoteOrderQty = Number(orderCalc.quoteAmount).toFixed(8);
            }
            else {
                orderParams.quantity = Number(orderCalc.quantity).toFixed(8);
            }
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (error.response?.data?.msg) {
                    return {
                        success: false,
                        error: `Erro MEXC: ${error.response.data.msg}`
                    };
                }
            }
            logger.error('Erro ao executar trade na MEXC', {
                error: error instanceof Error ? error.message : 'Erro desconhecido',
            ...
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Erro desconhecido ao executar trade'
            };
        }
    }
};

...
interface BotParams extends ParamsDictionary {
  id: string;
}
...
function createDefaultBot(userId: string): Bot {
  return {
    id: generateBotId(),
    name: '',
    enabled: false,
...
export function setupBotRoutes() {
  ...
  const listBots: RequestHandler = async (_req, res) => {
    try {
  ...
  const getBot: RequestHandler<BotParams> = async (req, res) => {
    try {
      const storage = ServerStorage.getInstance();
  ...
  const updateBot: RequestHandler<BotParams> = async (req, res) => {
    try {
      const storage = ServerStorage.getInstance();
  ...
  const toggleBot: RequestHandler<BotParams> = async (req, res) => {
    try {
      const storage = ServerStorage.getInstance();
      const bot = await storage.getBot(req.params.id);
  ...
  const deleteBot: RequestHandler<BotParams> = async (req, res) => {
    try {
      const storage = ServerStorage.getInstance();
      const bot = await storage.getBot(req.params.id);

import { Bot, WebhookPayload, LogAction } from './types';
...
export async function processWebhookRequest(
  ...
  try {
    ...
    const orderCalc = await calculateOrderQuantity(
      ...
      typeof payload.order_size === 'string' ? payload.order_size : String(payload.order_size),
      payload.ticker,
      await exchange.getCurrentPrice(payload.ticker),
      payload.action.toLowerCase() as 'buy' | 'sell'
    );
    ...
    const tradeResult = await exchange.executeTrade(credentials, {
      ...
      type: payload.action.toLowerCase() as 'buy' | 'sell',
    ...
    await addLog({
      ...
      action: payload.action.toLowerCase() as 'buy' | 'sell',

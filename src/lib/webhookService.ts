...
type WebhookOrder = z.infer<typeof OrderSchema>;

// Fila de processamento de ordens
const orderQueue = new Queue<WebhookOrder>();
 ...
 */
function validateOrderSignature(order: Omit<WebhookOrder, 'signature'>, signature: string, secretKey: string): boolean {
 ...
 */
async function processOrder(order: WebhookOrder): Promise<boolean> {
  try {
 ...
 */
export async function handleWebhookOrder(payload: unknown): Promise<{
  success: boolean;
  message?: string;
  error?: string;
}> {
  try {
    ...
    if (!validateOrderSignature(
      ...
      order.signature,
      bot.exchange.credentials.secretKey
    )) {
      return {
        success: false,

...
export async function validateMexcApiKeys(apiKey: string, secretKey: string): Promise<{ isValid: boolean; balances?: MexcBalance[]; error?: string }> {
  try {
...
export async function getAvailablePairs(): Promise<{ 
  ...
  try {
    ...
    const response = await axios.get<MexcExchangeInfo>(`${MEXC_API_URL}/exchangeInfo`);
